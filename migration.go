...
func (m *Migration) run(db *sql.DB, direction bool) error {
	...
	case ".sql":
		...
		statements, _, err := parseSQLMigration(f, direction)
		if err != nil {
		...
		if err := runSQLMigration(tx, statements, m.Version, direction); err != nil {
	...
	case ".go":
		if !m.Registered {
			return errors.Errorf("ERROR %v: failed to run Go migration: Go functions must be registered and built into a custom binary (see https://github.com/sillydong/goose/tree/master/examples/go-migrations)", m.Source)
		}
		tx, err := db.Begin()
		if err != nil {
